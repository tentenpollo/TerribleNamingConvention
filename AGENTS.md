...
Run from the repository root, not from backend/. The `pythonpath = ["backend"]` in `pyproject.toml` handles the import path.
...
- **Wrong worker module** — the real worker is backend/app/workers/settings.py. The root-level /workers/main.py is vestigial and processes nothing. (Or just delete /workers/ — it's two dead files.)
